...
def fetch_biocoop_data():
    try:
        ...
        cursor.execute("SELECT DISTINCT(name) FROM carrefour;")
...
def fetch_biocoop_data():
    try:
        ...
        # Query to fetch data
        cursor.execute("SELECT DISTINCT(product_name) FROM carrefour_section;")
        rows = cursor.fetchall()
...
try:
    ...
    sql = """
            INSERT INTO carrefour_section (product_name, section)  
            VALUES (%s, %s)
    """
