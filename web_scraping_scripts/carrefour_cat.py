...
def fetch_biocoop_data():
    try:
        ...
        # Query to fetch data
        cursor.execute("SELECT DISTINCT(product_name), section FROM carrefour_section;")
...
def fetch_biocoop_data():
    try:
        ...
        # Query to fetch data
        cursor.execute("SELECT DISTINCT(product_name) FROM carrefour_cat;")
        rows = cursor.fetchall()
...
def insert_auchan_categories(df, DB_PARAMS, verbose=True):
    ...
    try:
        ...
        sql = """
            INSERT INTO carrefour_cat (product_name, category)  
            VALUES (%s, %s)
        """
