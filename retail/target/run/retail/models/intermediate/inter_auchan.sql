  ...
  create  table "airflow"."public_intermediate"."inter_auchan__dbt_tmp"
...
-- Get min and max date per image_url + name
image_url_dates AS (
    SELECT
        image_url,
        name,
    ...
    FROM "airflow"."public_staging"."stg_auchan"
...
joined AS (
    ...
    LEFT JOIN "airflow"."public_staging"."stg_auchan" a
...
base_case AS (
    ...
    (
        ...
        FROM ordered
        WHERE name IS NOT NULL OR price IS NOT NULL OR stock IS NOT NULL
        ORDER BY image_url, name, date_day DESC
    )
),
